...
export interface CoffeeToolEntry<TUI extends Record<string, string> = Record<string, string>> {
  id: string;
  icons: {
    bg: string;
    fg: string;
...
export interface CoffeeCategoryEntry {
  icon: string;
  tools: CoffeeToolEntry[];
  i18n: LocaleMap<CategoryLocaleContent>;
}

export interface ToolDefinition {
  entry: CoffeeToolEntry;
  Component: unknown;
