...
function isMidRatioColon(text: string, colonPos: number): boolean {
  return (
    colonPos > 0 &&
    colonPos < text.length - 1 &&
    /\d/.test(text[colonPos - 1]) &&
    /\d/.test(text[colonPos + 1])
  );
}
...
function sentenceStart(text: string, endIndex: number): string {
  ...
  const colonCandidates = [...beforeMark.matchAll(/:/g)].map((m) => m.index ?? -1).filter((i) => i >= 0);
  const lastColon = colonCandidates.findLast((pos) => !isMidRatioColon(beforeMark, pos)) ?? -1;
